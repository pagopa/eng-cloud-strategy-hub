...
resource "aws_iam_policy" "s3_internal_idp_apigw_proxy" {
  ...
  description = "Get Object in S3 object."
  policy      = data.aws_iam_policy_document.s3_internal_idp_bucket_apigw_proxy[0].json
}
...
resource "aws_iam_role_policy_attachment" "s3_internal_idp_apigw_proxy" {
  count      = var.deploy_internal_idp_rest_api ? 1 : 0
  role       = aws_iam_role.s3_internal_idp_apigw_proxy[0].name
  policy_arn = aws_iam_policy.s3_internal_idp_apigw_proxy[0].arn
}
...
module "rest_api_internal_idp" {
  ...
  body = templatefile(var.openapi_internal_idp_template_file,
    {
      server_url                   = var.domain_name
      uri                          = format("http://%s:%s", var.nlb_dns_name, "8082"),
      connection_id                = aws_api_gateway_vpc_link.apigw.id
      aws_region                   = var.aws_region
      lambda_apigateway_proxy_role = aws_iam_role.lambda_apigw_proxy.arn
      provider_arn                 = var.provider_arn
      s3_apigateway_proxy_role     = aws_iam_role.s3_internal_idp_apigw_proxy[0].arn
