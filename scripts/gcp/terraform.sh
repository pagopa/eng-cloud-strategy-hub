#!/usr/bin/env bash
#
# Purpose: Terraform wrapper for GCP roots in this repository.
# Usage examples:
...
base_dir="$PWD"
backend_tfvars=""
state_project_id="${TF_STATE_PROJECT_ID:-organization-443016}"
dry_run=false
cicd_mode=false
skip_init=false
...
resolve_env_context() {
  ...
  base_dir="./projects/${env_arg}"
  backend_tfvars="${base_dir}/backend.tfvars"

  [[ -d "$base_dir" ]] || die "Missing project directory '${base_dir}'"
...
configure_provider_context() {
  ...
  require_cmd "gcloud" "needed for GCP project checks"

  if ! is_cicd_mode; then
    active_account="$(gcloud auth list --filter=status:ACTIVE --format=value\(account\) 2>/dev/null || true)"
    [[ -n "$active_account" ]] || die "No active gcloud account found. Run 'gcloud auth login' before using this script."
  else
    info "CICD mode enabled: skipping interactive gcloud auth checks"
  fi

  run_cmd gcloud config set project "$state_project_id"
  export TF_VAR_project_target="$env_arg"
}
...
run_doctor() {
  ...
  if [[ "$env_arg" == "noenv" ]]; then
    ...
    if command -v gcloud >/dev/null 2>&1; then
      success "gcloud CLI available"
      if is_cicd_mode; then
        info "CICD mode enabled: skipping interactive gcloud auth validation"
      else
        active_account="$(gcloud auth list --filter=status:ACTIVE --format=value\(account\) 2>/dev/null || true)"
        if [[ -n "$active_account" ]]; then
          success "Active gcloud account: ${active_account}"
        else
