#!/usr/bin/env bash
#
# Purpose: Terraform wrapper for AWS roots in this repository.
# Usage examples:
...
resolve_env_context() {
  ...
  account_backend="./${env_arg}/backend.ini"
  account_yaml="./${env_arg}/authorization.yaml"
  env_dir="./env/${env_arg}"
  env_backend="${env_dir}/backend.ini"
  ...
  if [[ -f "$account_backend" && -f "$account_yaml" ]]; then
    ...
    if [[ -f "$account_backend" && ! -f "$account_yaml" ]]; then
      die "Missing ${account_yaml}"
    fi
    if [[ -f "$account_yaml" && ! -f "$account_backend" ]]; then
      die "Missing ${account_backend}"
    fi
    die "Missing env files for '${env_arg}'. Expected ./${env_arg}/backend.ini plus authorization.yaml, or ./env/${env_arg}/backend.ini"
  fi
...
configure_provider_context() {
  ...
  if is_cicd_mode; then
    info "CICD mode enabled: skipping interactive AWS credential checks"
    return 0
  fi

  if [[ -z "$aws_profile" ]]; then
    warn "No aws_profile configured in ${backend_ini}; skipping AWS profile validation"
    return 0
  fi
  ...
  if aws sts get-caller-identity --profile "$aws_profile" >/dev/null 2>&1; then
    success "AWS credentials validated for profile '${aws_profile}'"
    return 0
  fi
...
require_default_var_file() {
  ...
  if [[ "$config_mode" == "yaml_account" ]]; then
    return 1
  fi
...
resolve_var_files() {
  ...
  if [[ "$no_default_tfvars" == false ]]; then
    ...
    elif require_default_var_file; then
...
build_terraform_command() {
  ...
  if [[ "$config_mode" == "yaml_account" && "$env_arg" != "noenv" ]]; then
    command_args+=("-var=account_key=${env_arg}")
  fi
...
probe_lock_id() {
  ...
  if [[ "$config_mode" == "yaml_account" && "$env_arg" != "noenv" ]]; then
    probe_cmd+=("-var=account_key=${env_arg}")
  fi
...
run_doctor() {
  ...
  elif [[ "$config_mode" == "yaml_account" ]]; then
    info "Legacy yaml_account mode: default tfvars are optional"
  else
